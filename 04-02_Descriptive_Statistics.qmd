...
To gain a first overview of the data that we use for modelling, we here present the summary statistics and visual exploration of the data. The final dataset for modelling is the result of joining data from different sources that are provided in @sec-data-description. Furthermore, we applied several filters that are outlined in @sec-data-prep.

The final dataset for modelling contains `r nrow(energy_modelling)` observations (gas-heated buildings). As we consider data from the years 2016, 2018 and 2020, one building can be present in the dataset three times at maximum. The buildings are distributed over 20 municipalities of the Canton of Basel-Landschaft.
...
energy_modelling %>%
  count(entrance_municipality_name, name = "n_obs") %>%
  arrange(n_obs) %>%
  ggplot(aes(x = n_obs, y = reorder(entrance_municipality_name, n_obs))) +
...
As outlined in Chapter @sec-methodology, we will use both HEC and HEPI as target variables. @tbl-obs-per-year shows that the means of HEC and HEPI are both decreasing between 2016 and 2020.
