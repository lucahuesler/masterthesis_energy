...
```{r}
#| label: match-gwr-entrance
#| include: false

#> match building_heights
energy_matched <- energy_raw %>%
...
energy_filtered <- energy_matched %>%
  filter(energy_consumed_measured_mwh > 1,
         building_category == 1020,
         energy_usage_of_installation %in% c("Raumwärme", "Raumwärme + Warmwasser", NA)
         )
```

By applying the above filters, we obtain a dataset of about `r nrow(energy_clean)` observations. As the dataset contains measured gas consumption for the years 2016, 2018 and 2020, one building can be present three times. 

```{r}
energy_clean %>%
  select(egid, survey_year, energy_consumed_measured_mwh) %>%
  group_by(survey_year) %>%
  summarize(Total_measured_gas_consumption = sum(energy_consumed_measured_mwh, na.rm = TRUE)) %>%
  ggplot(aes(x = survey_year, y = Total_measured_gas_consumption)) +
  geom_col() +
  xlab("Survey year") +
  ylab("Total measured gas consumption (MWh)")
...
retrofit_all_match <- retrofit_all_clean %>%
  select(egid, admin_id, admin_project_type, admin_payment_year, property_address, property_street, property_number, property_zip, property_parcel_number) 


energy_clean_retrofit <- energy_clean %>%
...
energy_clean_model <- energy_clean %>%
  mutate(hec = (energy_consumed_measured_mwh*efficiency_of_installation) + (energy_production_solar_mwh) - (energy_consumed_hot_water_mwh),
         hepi = hec/gross_floor_area) %>%
  filter(hec > 0)

sum(energy_clean_model$hec, na.rm = T)
sum(energy_clean_model$energy_consumed_measured_mwh, na.rm = T)

hec_check <- energy_clean_model %>%
  select(egid, energy_consumed_measured_mwh, energy_usage_of_installation, hec, efficiency_of_installation ,energy_production_solar_mwh,
...
summary_table <- energy_clean %>%
  group_by(dplzname, building_class) %>%
  summarise(n = n()) %>%
...
plot_boxplot <- function(df, y_axis) {
  y_axis <- sym(y_axis)
  df %>%
    mutate(class = fct_reorder(dplzname, !!y_axis, .fun='median')) %>%
    ggplot( aes(x=reorder(dplzname, !!y_axis), y=!!y_axis, fill=dplzname)) + 
      geom_boxplot() +
      xlab("class") +
...
boxplot_construction_year <- plot_boxplot(energy_clean, "construction_year")
boxplot_building_area_m2 <- plot_boxplot(energy_clean, "building_area_m2")
boxplot_num_residents <- plot_boxplot(energy_clean, "num_residents")

boxplot_year_of_installation <- energy_clean %>%
...
hist_plot <- energy_clean %>%
  ggplot(aes(x = energy_consumed_measured_mwh)) +
...
community_plot <- energy_clean %>%
  ggplot(aes(x = meters_above_sealevel, y = energy_consumed_measured_mwh)) +
  geom_point() +
  facet_grid(rows = vars(municipality_code))
...
lm_form_fit <- 
  lm_model %>% 
  fit(energy_consumed_measured_mwh_log ~ gross_floor_area + coordinate_e + coordinate_n + survey_year, data = energy_clean_log)
...
test_res <- bind_cols(prediction, energy_test %>% select(egid, building_name, energy_consumed_measured_mwh_log))
```

```{r}
ggplot(test_res, aes(x = energy_consumed_measured_mwh_log, y = .pred)) + 
...
```{r}
ggplot(energy_clean_log, aes(x = log(energy_consumed_measured_mwh_log), y = log(energy_consumed_estimated_mwh))) + 
  #> Create a diagonal line:
  geom_abline(lty = 2) + 
  geom_point(alpha = 0.5) + 
  labs(y = "Predicted Energy use (log10)", x = "Measured energy use (log10)") +
