...
data_by_construction_period <- split(energy_modelling, energy_modelling$construction_period)

### Fit & predict

all_metrics_hec_subset_construction_period <- data.frame()

# Run AutoML for each construction period separately
models_by_construction_period <- lapply(names(data_by_construction_period), function(name) {
  
  # Get the name of the current element in the list
  df_name <- name
  
  # Retrieve the corresponding data frame
  df <- data_by_construction_period[[name]]
  ...
  for (i in 1:nrow(leaderboard)) {
    ...
    all_metrics_hec_subset_construction_period <- bind_rows(all_metrics_hec_subset_construction_period, metrics)
  }
...
construction_period <- map_dfr(models_by_construction_period, ~ data.frame(construction_period = .x$construction_period))
aggregated_error_curr_method <- map_dfr(models_by_construction_period, ~ data.frame(aggregated_error_curr_method = .x$aggregated_error_curr_method))
aggregated_error_best_model <- map_dfr(models_by_construction_period, ~ data.frame(aggregated_error_best_model = .x$aggregated_error_best_model))

aggregated_errors_construction_period <- construction_period |>
  bind_cols(aggregated_error_best_model) |>
  bind_cols(aggregated_error_curr_method)

# save to rds
saveRDS(models_by_construction_period, paste0("models/models_by_construction_period_", Sys.Date(), ".rds"))
