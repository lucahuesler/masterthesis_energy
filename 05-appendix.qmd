...
pacman::p_load(tidyverse, tidymodels, doParallel, skimr, plotly, sf, agua, gt, patchwork, ggridges, viridis, cowplot)
...
![HEPI density plot for single family houses per municipality](99-appendix-ridgline-sfh.png){#ridgeline-sfh fig-align="center" width="70%"}

![HEPI density plot for multi family houses per municipality](figures/99-ridgeline-mfh.png){#ridgeline-mfh fig-align="center" width="70%"}
...
```{r}
#| label: fig-retrofitting-effect
#| fig-cap: "Comparison of HEC before and after retrofitting."

retrofit_plot <- energy_modelling |>
  filter(retrofitted == "Yes") |>
  arrange(desc(egid)) |>
  distinct(egid, survey_year, retrofit_year_completion, hec) |>
  pivot_wider(names_from = survey_year, names_prefix = "HEC (kWh) ", values_from = hec) 

retrofit_plot_2016_2018 <- retrofit_plot |>
  ggplot((aes(`HEC (kWh) 2018` , `HEC (kWh) 2016` ))) +
           geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  labs(title = 'HEC of buildings retrofitted between 2016 and 2018')

retrofit_plot_2018_2020 <- retrofit_plot |>
  ggplot((aes(`HEC (kWh) 2018` , `HEC (kWh) 2020` ))) +
           geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  labs(title = 'HEC of buildings retrofitted between 2018 and 2020')

retrofit_plot_2016_2020 <- retrofit_plot |>
  ggplot((aes(`HEC (kWh) 2016` , `HEC (kWh) 2020`))) +
           geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  labs(title = 'HEC of buildings retrofitted between 2018 and 2020')
...
#| fig-cap: "Actual and predicted values."
#| fig-cap-location: bottom
#| warning: false

# Create a list of density plots
 density_plots <- lapply(seq_along(aml_hec_all_data_models[1:4]), function(i) {
   model_type <- aml_hec_all_data_models[[i]]$variable_selection
   create_density_plot(aml_hec_all_data_models, i, model_type)
 })

# actual vs predicted plot
 actual_predicted_plots <- lapply(seq_along(aml_hec_all_data_models[1:4]), function(i) {
    model_type <- aml_hec_all_data_models[[i]]$variable_selection
    create_actual_predicted_plot(aml_hec_all_data_models, i, model_type)
    })

# residual plots
 residual_plots <- lapply(seq_along(aml_hec_all_data_models[1:4]), function(i) {
   model_type <- aml_hec_all_data_models[[i]]$variable_selection
...
plot_grid(plotlist = plot_pages_list, ncol = 1)
```
...
# Create a list of density plots
plots <- lapply(seq_along(models_by_construction_period[1:4]), function(i) {
