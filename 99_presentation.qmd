...
all_hec_metrics <- read_csv("models/auto_ml_models/all_hec_metrics_20230408.csv") 
all_hec_metrics <- all_hec_metrics |>
  rename(Algorithm = Algorithm...2,
         algo = Algorithm...7)



all_hepi_metrics <- read_csv("models/auto_ml_models/all_hepi_metrics_20230408.csv")
all_hepi_subset_survey_year_metrics <- read_csv("models/auto_ml_models/all_metrics_hepi_subset_survey_year.csv")
all_hepi_subset_building_class_metrics <- read_csv("models/auto_ml_models/all_metrics_hepi_subset_building_class.csv")

hec_aml_leaderboard_all <- read_csv("models/auto_ml_models/hec_aml_leaderboard_all_2023-05-07.csv")
...
retrofit_per_year <- energy_modelling |>
  ...
  dplyr::mutate(retrofit_project_name = case_when(
    ...
    grepl("Wärmedämmung", retrofit_project_name) ~ "Wärmedämmung",
...
retrofit_mean_hepi <- energy_modelling |>
  ...
  rename(`Retrofit` = `Yes`, `Non-retrofit` = `No`, ) |>
...
-   Integrated if a building is stand alone or not (binary)

-   Integrated STATPOP indicators

    -   Youth ratio (below 15)

    -   Elderly ratio (above 65)

    -   Ratio of 1 person households

    -   Ratio of residence less than 1 year

    -   Ratio of foreigners

## Recap II

Modelling: Focus on Auto ML (h2o)

-   Results better

-   Training with different predictor variables ([Step 1](https://lucahuesler.quarto.pub/estimating-the-heat-energy-use-of-residential-buildings/#/modelling-strategy))

-   Training on subsets ([Step 2](https://lucahuesler.quarto.pub/estimating-the-heat-energy-use-of-residential-buildings/#/modelling-strategy-1))

## Results: Influence of new variables

Train models with four different settings of variable selection:

-   With all predictors

-   Without retrofit predictor

-   Without social indicators (youth ratio, elderly ratio, etc.)

-   Without variable for "stand alone"

## Results: Influence of new variables

Top 10 models (based on RMSE **train**): What variable selection performs best?
...
hec_aml_leaderboard_all %>%
  ...
  ungroup() |>
  slice_head(n = 10) %>%
  select(approach, algo, rmse, mae, rmsle) |>
  mutate_if(is.numeric, round, 2) |>
  DT::datatable()
```

![](figures/presentation/hec_aml_leaderboard_train.png)

## Results: Influence of new variables

Top 10 models (based on RMSE **test**): What variable selection performs best?
...
all_hec_metrics %>%
  ...
  ungroup() |>
  slice_head(n = 10) %>%
  select(Approach, Algorithm, R_squared, RMSE, MAE, MAPE) |>
  mutate_if(is.numeric, round, 2) |>
  DT::datatable()
```

![](figures/presentation/hec_aml_leaderboard_test.png)

## Observations

-   Best three models (RMSE) are without retrofit, stand alone or social indicators. Hypothesis social indicators: They do not improve the model because they are on hectare level.

-   Model perfomance gets worse when removing retrofit information.

-   When considering MAE, which is robust to outliers, the leaderboard changes (but still models without social indicators perform best).
...
```{r}
options(scipen = 999)
all_hec_metrics_clean <- all_hec_metrics %>%
...
::: {.column width="40%"}
-   With exception of GLM, all models perform better than current approach

-   XGBoost shows best values for all metrics, but not very big differences of the first 3
:::
:::
