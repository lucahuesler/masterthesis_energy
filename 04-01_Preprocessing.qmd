...
# crop to BL
box = c(xmin = 2590110, ymin = 1242580, xmax = 2639810, ymax = 1268980)

stat_pop_2016_sf_bl <- stat_pop_2016_sf |>
  st_crop(box)
...
matched = st_join(energy_stat_pop_2016, stat_pop_2016_sf, join = nngeo::st_nn, maxdist = 100, k = 1, progress = TRUE)
View(matched)
...
```{r}
energy_points_2016_sf <- energy_points_2016 |>
  st_as_sf(coords = c("gkode", "gkodn"), crs = 4326)

stat_pop_2016_sf <- stat_pop_2016 |>
  st_as_sf(coords = c("e_koord", "n_koord"), crs = 4326)


energy_points_2016_statpop <- energy_points_2016_sf |>
  st_join(stat_pop_2016_sf, join = st_nearest_feature)
...
energy_clean_09 <- energy_clean_08 |>
  ...
  mutate(admin_id = if_else(retrofitted == 1, admin_id, NA_character_), 
         admin_project_type = if_else(retrofitted == 1, admin_project_type, NA_character_), 
         admin_project_name = if_else(retrofitted == 1, admin_project_name, NA_character_), 
         param_investment_costs = if_else(retrofitted == 1, param_investment_costs, NA_real_),
         year_completion = if_else(retrofitted == 1, year_completion, NA_integer_)) |>
...
write_rds(energy_clean_16, "data/energy_modelling.rds")
write_delim(energy_clean_16, "data/energy_modelling.csv")
```
