...
```{r}
#| label: preprocessing-recipes-hec
#| include: false

# Specify recipe for basic MLR
lm_base_recipe <- recipe(hec ~ .,
                          data = energy_train) %>% 
  # remove egid and hepi from the list of predictors
  update_role(egid, new_role = "id") %>%
  update_role(hepi, new_role = "id") %>% 
...
wf_set_basic <- 
   workflow_set(
      preproc = list(simple = lm_base_recipe, log = lm_log_recipe),
      models = list(lm = lm_spec, dt = dt_spec, rf = rf_spec),
      cross = TRUE
   )
...
#| include: false
set.seed(123)
splits <- vfold_cv(energy_train, v = 2, strata = hec)
...
best_results <- 
   fit_basic %>% 
   extract_workflow_set_result("log_rf") %>% 
...
prediction <- 
   fit_basic %>% 
   extract_workflow("log_rf") %>% 
   finalize_workflow(best_results) %>% 
   last_fit(split = energy_split)

collect_metrics(prediction)
```

```{r}
lm_test_results_plot <- prediction %>% 
   collect_predictions() %>% 
...
lm_form_fit <- 
  lm_model %>% 
  fit(hepi ~ ., data = energy_train)


summary(lm_form_fit$fit)
```
