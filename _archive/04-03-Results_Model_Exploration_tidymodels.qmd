...
auto_ml_results <- read_delim("models/auto_ml_models/all_hec_metrics_20230408.csv")
```

### Results Step 1: Comparing different models {#sec-model-exploration}

According to the modelling workflow defined in @sec-modelling-worflow, the first step of the modelling process focuses on the exploration of different models. To do so, we run the algorithms defined in @sec-algorithms manually but also with automated machine learning (AutoML)[^04-03-results_model_exploration-1]. We here present the results of the manual application of the algorithms, followed by the results obtained from the AutoML approach.

[^04-03-results_model_exploration-1]: For automated machine learning, the open-source AutoML library of H20 was used via interface (see also https://h2o.ai/platform/h2o-automl/).
...
autoplot(
  ...
  metric = c("rmse", "mae", "mape", "rsquared"),       # <- metrics to visualize
  ...
  ggtitle("Benchmark") +
  ylab("Value") +
  facet_wrap(~ metric, scales = "free_y")  # <- add facet_wrap for metrics
...
The AutoML approach takes care of data preprocessing, feature engineering and model selection. Thus, this approach requires much less manual work throughout the modelling process.

```{r}
#| label: automl-test-rsquared
#| include: false
options(scipen = 999)
auto_ml_results <- auto_ml_results %>%
  mutate(Algorithm = if_else(Algorithm == "Based on HEPI", "Current Approach", Algorithm))

best_r2_per_algo <- auto_ml_results %>%
  group_by(Algorithm) %>%
   dplyr::slice_max(order_by = R_squared, n = 1) |>
...
```
```{r}
#| label: automl-test-mae
#| include: false
#| 
best_mae_per_algo <- auto_ml_results %>%
  group_by(Algorithm) %>%
...
# Create the plot
ggplot(best_rmse_per_algo_long, aes(x = Algorithm, y = Value, fill = Algorithm)) +
