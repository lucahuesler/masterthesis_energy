...
retrofit_mean_hepi <- energy_modelling |>
  ...
  rename(`Retrofitted` = `TRUE`, `Not retrofitted` = `FALSE`, ) |>
...
normalized <- 
   workflow_set(
      ...
      models = list(neural_network = nnet_spec#SVM_radial = svm_r_spec, SVM_poly = svm_p_spec, 
                    #KNN = knn_spec
                    )
   )
...
no_pre_proc <- 
   workflow_set(
      ...
      models = list(#MARS = mars_spec, CART = cart_spec, CART_bagged = bag_cart_spec,
                    RF = rf_spec, boosting = xgb_spec, lm_basic = lm_basic_spec #Cubist = cubist_spec
                    )
   )
no_pre_proc
```
