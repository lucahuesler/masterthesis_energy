...
stat_pop_2016 <- stat_pop_2016 %>%
  mutate(geom_point = st_as_sf(., coords = c("E_KOORD", "N_KOORD"), remove = FALSE)) %>%
  mutate(geom_hectare = sf::st_buffer(geom_point, dist = 1, nQuadSegs=1, 
                        endCapStyle = "SQUARE"))
...
write_rds(energy_clean_09, "data/energy_modelling.rds")
```
