...
```{r}
# basic
hec_basic_aml_results <- run_h2o_automl(target = "hec", 
                                      predictors = predictors_basic, 
                                      data = train, 
                                      runtime = 360)

hec_basic_aml <- hec_basic_aml_results$aml
hec_basic_aml_leaderboard <- hec_basic_aml |>
  h2o.get_leaderboard(extra_columns = 'algo') |>
    as.data.frame() |>
  mutate(approach = "Basic")
...
```{r}
hec_aml_leaderboard_all <- hec_basic_aml_results |>
  bind_rows(hec_all_aml_leaderboard) |>
...
process_leaderboard <- function(df) {
  ...
  for (i in 1:nrow(df)) {
    ...
    all_hec_metrics <<- rbind(all_hec_metrics, metrics) 
...
```{r}
lapply(list(hec_basic_aml_leaderboard), process_leaderboard)
```
...
```{r}
hec_all_aml_best_model_test <- h2o.getModel("GBM_5_AutoML_8_20230512_153215")
preds <- h2o.predict(hec_all_aml_best_model_test, test)
h2o.explain_row(hec_all_aml_best_model_test, test, row_index = 10102)
# # 
h2o.explain(hec_all_aml_best_model_test, test)
# 
h2o.shap_summary_plot(hec_all_aml_best_model_test, test)
h2o.varimp(hec_all_aml_best_model_test)
```
